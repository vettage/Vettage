...
class content_model extends CI_Model {  
	...
	function remove_home_images($content_id=NULL)
	{
		...
		if($image!=NULL)
		{
			...
			$arg = explode('.',$image);
			echo $img_path = $path.$arg[0].'_689.'.$arg[1];
			if(file_exists($img_path)) unlink($img_path);
		}
		return;
	}
	...
	function get_allot_details()
	{
		...
		FROM contents a, content_ratings b WHERE (a.content_id=b.content_id) GROUP BY a.content_id ORDER BY AVG(importance)+AVG(credibility)+AVG(timeline)+AVG(appearance) DESC");
		return $query->result();
	}
