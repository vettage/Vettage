...
class content_model extends CI_Model {  
	...
	function search($terms){
		...
		$where = "1=1 ";
		if (!empty($terms['city'])) $where .= " and city='".$terms['city']."'";
		if (empty($terms['city']) && !empty($terms['state'])) $where .= " and state='".$terms['state']."'";
		if (empty($terms['city']) && empty($terms['state']) && !empty($terms['country'])) $where .= " and country='".$terms['country']."'";
		
		$sql= "select * from contents where $where";
	...
	{	//print_r($data);exit;
		...
		// why would you not filter out columns?
		unset($data['location']);
	...
	function get_keywords($fields="*",$str_cond)
	{
		
		if (empty($str_cond)) $query = $this->db->query("select * from contents");
			else $query = $this->db->query("select * from contents where 1=1 $str_cond");
		return $query->result();
	}
	
	function get_tags($fields="*",$str_cond)
	{
		$query = $this->db->query("select tags from contents");
		return $query->result();
	}
