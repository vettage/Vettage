...
class Member_model extends CI_Model {  
	...
	{
		//echo "select $str_fields from members $str_cond ";exit;
		$query  = $this->db->query("select $str_fields from members $str_cond ");
		if($query->num_rows()>0)
	...
	function combox($fields="*",$str_cond="1"){
        $query = $this->db->query("select $fields from members where $str_cond");
		return $query->result();
	}
	
	function get_single_record($cond)
	{	
		$query_row  = $this->db->query("select * from members where $cond");
	...
	{ 
		...
		$this->db->update('members',$data,$cond);
		return;
	}
	
	function edit_member($data,$mem_id)
	{
		$this->db->update('members',$data,"mem_id = ".$mem_id);
		return;
	}
	...
	function delete($field,$id)
	{
		...
		$query = $this->db->delete('members');/* delete user master table */
		return 1;
	}
	...
	{	
		if($mem_id)
		{
			$query = $this->db->where('mem_id', $mem_id);
      		...
      		$query = $this->db->delete('members');/* delete re_users master table */
			return 1;
		}
	}
	...
	function activate_user($cond)
	{
		...
		$user_data['status '] =1;
		$this->db->update('members',$user_data,"verification_code ='$cond' ");
	}
	...
	function change_status($mem_id)
	{
		...
		$this->db->update('members',$user_data,"mem_id = ".$mem_id);
		return 1;
	}
	...
	{
		$query_row  = $this->db->query("select $field from members where mem_id=$id");
	...
	function get_level($mem_id)
	{
		...
		{
			$query = 'select level from members where mem_id ='.$mem_id;
			$query_row  = $this->db->query($query);
			$query_data = $query_row->row();
