...
class User_model extends CI_Model {  
	...
	{
		//echo "select $str_fields from users $str_cond ";exit;
		$query  = $this->db->query("select $str_fields from users $str_cond ");
		if($query->num_rows()>0)
	...
	function combox($fields="*",$str_cond="1"){
        $query = $this->db->query("select $fields from users where $str_cond");
		return $query->result();
	}

	function location_combox($fields="*",$str_cond="1"){
		$query = $this->db->query("select distinct(U.id), U.$fields from users as U left join location as L on U.id=L.user_id where $str_cond group by U.id");
		return $query->result();
	}
	...
	{	
		$query_row  = $this->db->query("select * from users where $cond");
	...
	{ 
		...
		$this->db->update('users',$data,$cond);
		return;
	}
	
	function edit_member($data,$user_id)
	{
		return $this->edit_user($data,$user_id);
	}
	...
	function delete($field,$id)
	{
		...
		$query = $this->db->delete('users');/* delete user master table */
		return 1;
	}
	...
	{	
		if($mem_id)
		{
			$query = $this->db->where('mem_id', $mem_id);
      		...
      		$query = $this->db->delete('users');/* delete re_users master table */
			return 1;
		}
	}
	...
	function activate_user($cond)
	{
		...
		$user_data['status '] =1;
		$this->db->update('users',$user_data,"verification_code ='$cond' ");
	}
	
	function change_status($user_id)
	{
		$record = $this->get_single_data("id=$user_id");
	...
	function close_account_status_closed($user_id)	// acoount closed status
	{
		$user_data['status'] =  2;
		$this->db->update('users',$user_data,"id = ".$user_id);
		return 1;
	...
	{
		$_FILES = $image;
		$config['upload_path'] = getcwd()."/media/uploads/members/";
		...
		else
		{
			...
			//resize_32
			$config['source_image'] = getcwd()."/media/uploads/members/".$raw_name.$file_ext;
			$config['new_image'] =  getcwd()."/media/uploads/members/".$raw_name.'_32'.$file_ext;
			...
			//resize_100
			$config['source_image'] = getcwd()."/media/uploads/members/".$raw_name.$file_ext;
			$config['new_image'] =  getcwd()."/media/uploads/members/".$raw_name.'_100'.$file_ext;
			$config['maintain_ratio'] = FALSE;
			$config['width'] = 100;
			$config['height'] = 100;
			$this->image_lib->initialize($config); 
			...
			//resize_210
			$config['source_image'] = getcwd()."/media/uploads/members/".$raw_name.$file_ext;
			$config['new_image'] =  getcwd()."/media/uploads/members/".$raw_name.'_210'.$file_ext;
			$config['maintain_ratio'] = FALSE;
			$config['width'] = 210;
			$config['height'] =140;
			$this->image_lib->initialize($config); 
	...
	function get_image($mem_id="*",$image="randombig.gif",$size=210)
	{
		...
		{
			$path = getcwd()."/media/uploads/members/";
			$img_path = $path.$image;
			if(file_exists($img_path))
			{
				$arg = explode('.',$image);
				$img_name = $arg[0]."_$size.".$arg[1];
				return '<img src="'.BASE_ASSETS.'uploads/members/'.$img_name.'">';
			}
		}
		return '<img src="'.BASE_ASSETS.'uploads/members/randombig.gif" width="'.$size.'">';
	}
	...
	{
		$query_row  = $this->db->query("select $field from users where id=$id");
	...
	{
		$query = 'select level from member_upgrades where mem_id ='.$mem_id.' and upgrade_id = (select max(upgrade_id) from member_upgrades where mem_id ='.$mem_id.')';
		$query_row  = $this->db->query($query);
		$query_data = $query_row->row();
		
		if($query_row->num_rows()>0)
			return $query_data->level;
		else
		{
