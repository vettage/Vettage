...
class Contents extends admin_template{  
	...
	function index()
    {
		...
		if($_GET)
		{
			if($_GET['action']=="Search")
			{
				if(!empty($_GET['field']) && !empty($_GET['keyword']))
				{
					...
					{
						$mem_id=0;
						$details = $this->member_model->get_single_record("username='".$keyword."'");
						if($details!=NULL) 
						{
							if($details->type==1)
							{
								...
								$arr_cond.= " AND editor = '".$details->mem_id."'" ;
						}
						else
		...
		$arr_cond.= ' ORDER BY content_id DESC,image DESC limit '.$page.','.$config['per_page'];
