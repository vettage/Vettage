		...
		{ //print_r($_GET);exit;
			...
			{	
				...
				{ 
					...
					Like '".date("Y-m")."%' AND username LIKE '".$_GET['username']."' ORDER BY content_id DESC");
					/*echo "SELECT allotment.content_id FROM allotment LEFT JOIN members ON allotment.contributor_id=members.mem_id 
					LEFT JOIN contents ON allotment.content_id=contents.content_id WHERE story_date 
					Like '".date("Y-m")."%' AND username LIKE '".$_GET['username']."' ORDER BY content_id DESC";*/
			...
			if(!empty($_GET['searchtypes']) && $_GET['searchtypes']!='')
			{
				...
				for($i=0;$i<sizeof($type_array);$i++)
				{
					...
					{
						$searchtypestring.=  " types LIKE '%".trim($type_array[$i])."%' ";
						if($i!=(sizeof($type_array)-1)) {
							if($type_array[$i]!= 'Animation' && $type_array[$i]!= 'Editor')
					         {
							$searchtypestring.=  " OR ";
							}
						}
...
*/
	...
	{	
		...
		{    
			...
			$data['story_data'] = $this->content_model->custom_query("");	
