...
<div class="container">
		...
		<tr>
			...
			<td <?php if($level==1){?>style="color:#fff;background:#357EBD"<?php } ?>>
				<?php if($level<=1){?><input type="radio" name="level" value="1" id="level_1" class="pay_level"/> <?php }?>
			</td>
			<td <?php if($level==3){?>style="background:#357EBD"<?php } ?>>
				<?php if($level<=3){?><input type="radio" name="level" value="3" id="level_3" class="pay_level"/><?php }?>
			</td>
			<td <?php if($level==4){?>style="background:#357EBD"<?php } ?>>
				<?php if($level<=4){?><input type="radio" name="level" value="4" id="level_4" class="pay_level"/><?php }?>
			</td>
			<td <?php if($level==5){?>style="background:#357EBD"<?php } ?>>
				<?php if($level<=5){?><input type="radio" name="level" value="5" id="level_5" class="pay_level"/><?php }?>
			</td>
		</tr>
		</tbody>
    </table>
	...
	<div class="row">
		<div class="col-lg-5">
			<p>Journalism is about serving the public interest.</p>
			<p>When you subscribe, you do so because you are fed up with media bias, special interest influence, and you want the kind of quality reporting which has been lost to budget cuts and ad-driven infotainment.</p>
			<p>Your ratings ensure the reporting on Vettage.com is important, accurate, timely and aesthetically, emotionally and intellectually engaging.</p>
			<p>Your decisions are too important to be based on anything less.</p>
		</div>
		<div class="col-lg-7 bg-light pad-top pad-bottom">
			...
			<hr><?php */?>
            <hr>
			<div class="row" style="display:none" id="bitwall_div">
				<div class="col-lg-4">
					<label class="checkbox">
...
$('.pay_level').change(function(event){
  var id = event.target.id;
  if(id == "level_2") $('#bitwall_div').css('display','block');
  if(id == "level_3") $('#bitwall_div').css('display','none');
  if(id == "level_4") $('#bitwall_div').css('display','none');
  if(id == "level_5") $('#bitwall_div').css('display','none');
  if(id == "level_1") $('#bitwall_div').css('display','none');
});
